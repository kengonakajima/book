...
namespace vce
{
	...
	template <class T1, class T2>bool operator==(const Altr<T1>&a, const Altr<T2>&b) throw() { return a.mm==b.mm; }
	...
	template <typename A,typename B>struct g_map:public std::map<A,B,std::less<A>,GlobalAltr<std::pair<const A,B> > >{};
	...
	{
		//!@ja 重要なログ。少なめ。 @endja
		VCE_LOGLEVEL_IMPORTANT = 1,
		//!@ja VCEの内部動作のログ。やや多い。 @endja
		VCE_LOGLEVEL_INTERNAL = 2,
		//!@ja OSレベルのAPI呼び出しなどのログ。とても多い。VCE のデバッグ向け。 @endja
		VCE_LOGLEVEL_FOR_DEBUGGING_VCE = 4,

		VCE_LOGLEVEL_LOW = VCE_LOGLEVEL_IMPORTANT,
		VCE_LOGLEVEL_MEDIUM = VCE_LOGLEVEL_INTERNAL,
		VCE_LOGLEVEL_HIGH = VCE_LOGLEVEL_FOR_DEBUGGING_VCE,
	};

	inline const char * ExceptionToString(vce::VCE_EXCEPTION e)
	{
		switch ( e )
		{
		case vce::VCE_EXCEPT_UNKNOWN:
			return "VCE_EXCEPT_UNKNOWN";
		case vce::VCE_EXCEPT_CREATESOCKETFAIL:
			return "VCE_EXCEPT_CREATESOCKETFAIL";
		case vce::VCE_EXCEPT_BINDFAIL:
			return "VCE_EXCEPT_BINDFAIL";
		case vce::VCE_EXCEPT_LISTENINGFAIL:
			return "VCE_EXCEPT_LISTENINGFAIL";
		case vce::VCE_EXCEPT_NONBLOCKING:
			return "VCE_EXCEPT_NONBLOCKING";
		case vce::VCE_EXCEPT_CONNECTFAIL:
			return "VCE_EXCEPT_CONNECTFAIL";
		case vce::VCE_EXCEPT_CONNECTTIMEOUT:
			return "VCE_EXCEPT_CONNECTTIMEOUT";
		case vce::VCE_EXCEPT_CONNECTREFUSED:
			return "VCE_EXCEPT_CONNECTREFUSED";
		case vce::VCE_EXCEPT_NAMERESOLUTIONFAIL:
			return "VCE_EXCEPT_NAMERESOLUTIONFAIL";
		case vce::VCE_EXCEPT_KEYEXCHANGEFAIL:
			return "VCE_EXCEPT_KEYEXCHANGEFAIL";
		case vce::VCE_EXCEPT_RECVBUFFER_FULL:
			return "VCE_EXCEPT_RECVBUFFER_FULL";
		case vce::VCE_EXCEPT_SENDBUFFER_FULL:
			return "VCE_EXCEPT_SENDBUFFER_FULL";
		case vce::VCE_EXCEPT_SIGPIPE:
			return "VCE_EXCEPT_SIGPIPE";
		case vce::VCE_EXCEPT_INVALIDDATA:
			return "VCE_EXCEPT_INVALIDDATA";
		case vce::VCE_EXCEPT_TIMEOUT:
			return "VCE_EXCEPT_TIMEOUT";
		case vce::VCE_EXCEPT_TOOMANYFILES:
			return "VCE_EXCEPT_TOOMANYFILES";
		case vce::VCE_EXCEPT_ALREADYLISTEN:
			return "VCE_EXCEPT_ALREADYLISTEN";
		case vce::VCE_EXCEPT_CONNECTIONRESET:
			return "VCE_EXCEPT_CONNECTIONRESET";
		case vce::VCE_EXCEPT_OVERFLOWSENDQUEUE:
			return "VCE_EXCEPT_OVERFLOWSENDQUEUE";
		case vce::VCE_EXCEPT_OVERFLOWSENDBUFFER:
			return "VCE_EXCEPT_OVERFLOWSENDBUFFER";
		default:
			return "unknown";
		}
	}

	inline const char * CloseReasonToString(vce::VCE_CLOSEREASON reason)
	{
		switch ( reason )
		{
		case vce::VCE_CLOSE_UNKNOWN:
			return "VCE_CLOSE_UNKNOWN";
		case vce::VCE_CLOSE_LOCAL:
			return "VCE_CLOSE_LOCAL";
		case vce::VCE_CLOSE_REMOTE:
			return "VCE_CLOSE_REMOTE";
		case vce::VCE_CLOSE_RECVBUFFERFULL:
			return "VCE_CLOSE_RECVBUFFERFULL";
		case vce::VCE_CLOSE_RECVERROR:
			return "VCE_CLOSE_RECVERROR";
		case vce::VCE_CLOSE_INVALIDDATA:
			return "VCE_CLOSE_INVALIDDATA";
		case vce::VCE_CLOSE_HANDSHAKINGFAIL:
			return "VCE_CLOSE_HANDSHAKINGFAIL";
		default:
			return "unknown";
		}
	}
	...
	//
	typedef void * (*SelectAlgorithmCallback)(vce::VCE * api, vce::Session * s, vce::Listener * l, void * userdata);

	/*!@ja 古いログの方式を用いることのできる互換性保持のための関数群
	新しい方式との併用はできません。
	@deprecated 次回のリリース時にはこの関数群は使えなくなりますので、その時までに新しいログの方式に移行していただく必要があります。ご注意ください。
	@sa vce::SetLogCallback
	@endja
	*/
	namespace old_logsystem
	{
		//!@ja VCEオブジェクトの生成 @endja
		/*!@ja 
		この関数でまずVCEオブジェクトを作ってAPIを使えるようにする必要があります。複数作ることもできます。
		同時にログをセットすることもできます。
		vce::VCECreate の代わりにご使用ください。
		@se vce::VCECreate
		@deprecated 次回のリリース時にはこの関数は使えなくなりますので、その時までに新しいログの方式に移行していただく必要があります。ご注意ください。
		@endja
		*/
		...
		@param api 古い方式で VCE オブジェクトのメンバだったこの関数は、互換関数では引数で VCE オブジェクトのポインタを与える必要があります。
		@deprecated 次回のリリース時にはこの関数は使えなくなりますので、その時までに新しいログの方式に移行していただく必要があります。ご注意ください。
		@endja
		*/
		void SetLogCallback(VCE * api, void(*callback)(const char *log, int level));
        ...
        @endja
        */
        void SetLogger(VCE * api, std::ostream * target, int level = VCE_LOGLEVEL_LOW);
		...
		@endja
		*/
        std::ostream *Log(VCE * api, int level = 0);
	}
    ...
    */
    ...
    public:
		...
		*/
		...
		/*!@ja
		select に依存しないシステムになり、この関数の利用による影響はまったくなくなりました。
		@param limit 制限値
		@deprecated 廃止予定です。
		@endja
		*/
		...
		//!@ja 使用するアルゴリズムの選択を行うコールバック関数を設定します @endja
		/*!@ja
		TCP/UDP、暗号化・圧縮の方式など、アルゴリズムの選択を行う場合に設定するための関数を用意し、その関数ポインタを登録します。
		@param callback コールバック関数ポインタ
		@sa SelectSession
		@endja
		*/
		virtual void SetSelectAlgorithmCallback(SelectAlgorithmCallback callback)=0;

        //!@ja 接続します。 @endja
        /*!@ja 
        セッションsを使いhostname:portに接続します。
		接続先をホスト名で指定した場合は DNSThread という名前解決のスレッドが起動します。このスレッドはすべての VCE オブジェクトが削除されるときに終了されます。
		接続先を IP アドレスで指定した場合は DNSThread は起動しません。シングルスレッドで使用したい場合はこの方法で指定してください。
		DNSThread を使わず名前解決も行いたい場合は、vce::GetAddressByHostname 関数を使用してください。
		...
		@param bindaddress 接続するときに使うインターフェイスのアドレス。GetSelfAddressで返された値か、直接32ビットのIPアドレスを指定します。
        @return ユニークID、失敗した場合は0
        @sa Session
		@sa Session::CancelConnecting
		@sa Listen
		@sa vce::AddrToString
		@sa vce::GetAddressByHostname
        @endja
        */
		virtual VUint32 Connect(Session *s,const char *hostname,VUint16 port,unsigned int timeout=0,VUint32 bindaddress=0)=0;
        virtual VUint32 Connect(Session *s,const wchar *hostname,VUint16 port,unsigned int timeout=0,VUint32 bindaddress=0)=0;
		//!@ja UDPで接続します。 @endja
		/*!@ja
		...
		@endja
		*/
		virtual VUint32 UdpConnect(Session *s,const char *hostname,VUint16 port,unsigned int timeout=0,VUint32 bindaddress=0)=0;
		virtual VUint32 UdpConnect(Session *s,const wchar *hostname,VUint16 port,unsigned int timeout=0,VUint32 bindaddress=0)=0;
		...
		*/
		 ...
		 * TODO: doc
		 */
		virtual vce::VUint32 BindStream(vce::Session *sess,vce::StreamBinding* stream,vce::LoopReaction* reaction) = 0;
        ...
        @param bindaddress リスニングするIPアドレスを指定します。NULLなら自分の持つ全アドレスになります。ホスト名を指定すると DNSThread が起動します。DNSThread を起動せずにホスト名を扱いたい場合は vce::GetAddressByHostname 関数を用いてください。
		@param timeout リスニングに失敗したときに試行する時間を指定します。
        @return ユニークID
        @endja
        */
		virtual VUint32 Listen(Listener *l,VUint16 port,vce::VUint32 timeout=0)=0;
		virtual VUint32 Listen(Listener *l,VUint16 port,const char *bindaddress,vce::VUint32 timeout=0)=0;
		virtual VUint32 Listen(Listener *l,VUint16 port,const wchar *bindaddress,vce::VUint32 timeout=0)=0;
		...
		@endja
		*/
		virtual VUint32 UdpListen(Listener *l,VUint16 port,vce::VUint32 timeout=0)=0;
		virtual VUint32 UdpListen(Listener *l,VUint16 port,const char *bindaddress,vce::VUint32 timeout=0)=0;
		virtual VUint32 UdpListen(Listener *l,VUint16 port,const wchar *bindaddress,vce::VUint32 timeout=0)=0;
		...
		@endja
		*/
		virtual VUint32 ProxyListen(Listener *l,VUint16 proxyport,const char *proxyaddress,const char *password="")=0;
		...
		*/
		...
		@endja
		*/
		virtual vce::VUint32 CreateFinder(vce::Finder *f,vce::VUint16 port=59820,const char *bindaddress="",vce::VUint16 range=10)=0;
        ...
        */
		...
		typedef g_vector<Session*> SessionList;
		...
		@endja
		*/
		virtual void GetAllSession(SessionList &v)=0;
		...
		*/
		...
		//!@ja ListenするときのAccept待ち数を設定します。 @endja
		/*!@ja
		ListenするときのAccept待ち数を設定します。短い時間で多くの接続をしたり、
		...
		@endja
		*/
		virtual void SetListenBacklog(unsigned int backlog)=0;

		//!@ja ListenするときのAccept待ち数を設定します。(互換性のための関数) @endja
		/*!@ja
		VCE2.3.0 以前のバージョンで用いることができた関数です。
		挙動は SetListenBacklog 関数と同じになります。
		@deprecated 互換性保持のためのもので、次回リリースで削除予定です。かわりに VCE::SetListenBacklog(unsigned int backlog) を使用してください。
		@sa SetListenBacklog
		@endja
		*/
		...
		*/
		...
		//!@ja ポール時間の掛かり過ぎを警告する時間を設定します。 @endja
		/*!@ja
		1 回のポール当たりに掛った時間が設定した値を超えると、vce::VCE_LOG_TYPE_VCE_POLLING_TIME_IS_TOO_LONG でログコールバック関数が呼び出されます。
		ログコールバック関数がデフォルトの場合はログが表示されます。
		使用条件によって適切な時間はさまざまです。
		@param time 警告を出す時間。ミリ秒。デフォルトは 500 です。
		@endja
		*/
		virtual void SetTimeForDullPolling(VUint64 time)=0;

#ifndef VCE_EMBEDDED_EDITION
		//!@ja 使用中ポートのバインド可能設定です。 @endja
		/*!@ja
		OS がすでに使用しているポート番号をバインドし、リッスンできるかどうかの設定を行います。
		デフォルトではバインドできます。
		不可能に設定すると、Listen 時に VCE_EXCEPT_BINDFAIL が発生します。
		組み込み版 VCE にはこの機能はありません。
		@param enable true だとバインドできるように、false だとバインドできないようになります。
		@endja
		*/
        virtual void SetBindableUsingPort(bool enable)=0;
#endif
		...
		@endja
		*/
		inline void SetLogCallback(void(*callback)(const char *log,int level))
		{
			old_logsystem::SetLogCallback(this, callback);
		}
        ...
        @endja
        */
        inline void SetLogger(std::ostream *target,int level=VCE_LOGLEVEL_LOW)
		{
			old_logsystem::SetLogger(this, target, level);
		}
		...
		@endja
		*/
        inline std::ostream *Log(int level=0)
		{
			return old_logsystem::Log(this, level);
		}
    ...
    */
	...
	@return 設定ファイルの内容 失敗した場合は空になります
    @endja
    */
    ...
    /*!@ja 
    name=value 形式の文字列をconf["name"]==valueにマップします。"#"記号以降と、"#"記号の前の空白、"="の前後の空白は無視されます。
	@param config 設定ファイルの内容 失敗した場合は空になります
    @param filename 設定ファイル
	@return 読み込みに成功したかどうか
    @endja
    */
	bool LoadSimpleConfig(ConfigMap& config, const std::string& filename);
    /*!internal
	 * 
	 */
	...
	*/
	...
	@endja
	*/
	vce::g_string MemoryToStr(const void *data,size_t len);

	//!@ja VCE 内の sizeof(void *) の値  @endja
	/*!@ja 
	@endja
	*/
	std::size_t SizeOfVoidPointer();
    ...
    */
    ...
    public:
		...
		*/
		...
		@param priority 2 以上の値を設定することで優先度を下げる事ができます。デフォルトは 0 です。
		@endja
		*/
    ...
    */
    ...
    public:
		...
		*/
		...
		@endja
		*/
        template <typename T> void GetChildren(vce::g_vector<T*> &ch)
        {
            ch.resize(GetChildren());
			if(ch.size())GetChildren((Session**)&ch[0],(unsigned int)ch.size());
        }

		typedef vce::g_map<vce::g_string,vce::g_string> ProxyConfigParameter;
		...
		@endja
		*/
		bool ProxyConfig(const char *function,const ProxyConfigParameter &parameter);
    ...
    */
    ...
    public:
        ...
        enum ProtocolType
        {
            ...
            PROTOCOL_TYPE_UDP,
			//!@ja ループバック @endja
            PROTOCOL_TYPE_LOOPBACK,
        };
		unsigned int protocol_type;//!<@ja プロトコルの種類 @endja

#ifndef VCE_EMBEDDED_EDITION
		// rtti emu
		class Codec *isCodec;
		class MPISession *isMPISession;
#endif
	private:
		//!@ja 接続状態 @endja
	...
	public:
        ...
        */
		...
		@endja
		*/
		vce::g_string GetRemoteHostname()const;
		...
		*/
		...
		@endja
		*/
		virtual bool UdpSetTimeoutTime(vce::VUint64 time);
		//!@ja UDPのみ。パケット再送時間設定 @endja
		/*!@ja
		相手からパケット受信の確認が無かったときにパケットを再送するまでの時間を設定します。
		Attachedコールバック後に設定することができます。
		@param time ミリ秒単位の時間。初期値は1000ミリ秒。
		@return 成功した場合はtrue。
		@endja
		*/
		virtual bool UdpSetResendTime(int time);
		//!@ja UDPのみ。鍵交換時のパケット再送時間設定 @endja
		/*!@ja
		鍵交換時に相手からパケット受信の確認が無かったときにパケットを再送するまでの時間を設定します。
		Attachedコールバック後に設定することができます。
		@param time ミリ秒単位の時間。初期値は1000ミリ秒。
		@return 成功した場合はtrue。
		@endja
		*/
		virtual bool UdpSetKeyExchangeResendTime(int time);

		//!@ja UDPのみ。パケット再送時間調整値設定 @endja
		/*!@ja
		再送したときに次に再送するまでの増加時間を設定します。
		例えば 2.0 を設定すると再送するたびに次の再送までの時間を 2.0 倍します。送信成功すると再送時間が戻ります。
		大きな遅延が発生しやすくなりますが、再送によるトラフィック増加を抑える事が出来、結果的に遅延が少なくなる事があります。
		@param scaler 1.0 以上。初期値は 1.0 です。
		@return 成功した場合はtrue。
		@endja
		*/
		virtual bool UdpSetResendTimeScaler(float scaler);
    ...
    protected:
        ...
        virtual Session *Accepter(VUint32,VUint16){
            if(maxconnections&&GetChildren()>=maxconnections)
                return NULL;
			return enable?api->ma->New<T>():NULL;};
    public:
		//!@ja コンストラクタ @endja
		/*!@ja 
	...
	*/
	...
	typedef vce::g_map<unsigned char,MPINode*> MPIMap;
	typedef vce::g_map<unsigned char,MPINode*>::iterator MPIMapItr;
...
//      int Merge(const void *p,size_t sz,unsigned char MPInum);
	...
	*/
    ...
    /*!@ja 
    この関数でまずVCEオブジェクトを作ってAPIを使えるようにする必要があります。複数作ることもできます。
	ログの設定は行えなくなりました。新しいログの方式をご使用いただくか、過去と互換性のある vce::old_logsystem::VCECreate (非推奨) をご使用ください。
	@sa vce::old_logsystem::VCECreate
    @endja
    */
    VCE * VCECreateInstance(MemoryAllocator *ma = &defaultallocator);

    //!@ja VCEオブジェクトの生成 @endja
    /*!@ja 
    この関数でまずVCEオブジェクトを作ってAPIを使えるようにする必要があります。複数作ることもできます。
	@sa vce::old_logsystem::VCECreate
	@sa vce::VCECreateInstance
	@deprecated 次回のリリース時にはこの関数は使えなくなりますので、その時までに新しいログの方式に移行していただく必要があります。ご注意ください。
    @endja
    */
	inline VCE *VCECreate(std::ostream *target = NULL, int loglevel = VCE_LOGLEVEL_LOW, MemoryAllocator * ma = &defaultallocator)
	{
		return old_logsystem::VCECreate(target, loglevel, ma);
	}
	...
	*/
	...
	//!@ja 同期名前解決 @endja
	/*!@ja
	DNS を用いてホスト名の解決を行い、IP アドレスを取得します。
	この関数を呼び出すと名前が解決されるまで関数内で停止しますので、非同期で行いたい場合は VCE::Connect, VCE::Listen などの
	アドレスにホスト名を指定してください。詳しくは VCE::Connect, VCE::Listen を参照ください。
	@param hostname ホスト名
	@return IP アドレス。数値表現のアドレスなので、"123.456.789.123" のような文字列に変換したい場合は vce::AddrToString を用いてください。
	@sa VCE::Connect
	@sa VCE::Listen
	@sa vce::AddrToString
	@endja
	*/
	vce::VUint32 GetAddressByHostname(const char * hostname);
			...
			if(!key["address"].empty()&&vce::atoi(key["port"]))
			{
				...
				for(size_t i=0;i<v.size();i++)
					if(v[i]->GetRemoteAddress(remoteaddr,remoteport)&&remoteaddr==addr&&remoteport==port)
						return;
				api->Connect(new Session,key["address"].c_str(),port,2000);
			}
		}
	public:
		//!@ja コンストラクタ @endja
	...
	*/
	...
	/*!@ja
	セッションで使用するアルゴリズムを llflags を参考に自動的に選択するコールバックのデフォルト機能を提供します。
	この関数は VCE::SetSelectAlgorithmCallback で指定してください。
	@sa Session::SelectAlgorithm
	@sa VCE::SetSelectAlgorithmCallback
	@endja
	*/
	void * DefaultSelectAlgorithm(vce::VCE * api, vce::Session * s, vce::Listener * l, void * userdata);

	//!@ja セッションの種類の選択を行う関数 @endja
	/*!@ja
	セッションで使用するアルゴリズムを選択するための関数群です。
	Session::SelectAlgorithmコールバックに、この中から1つを定義する関数を書いてください。
	詳しくは Session::SelectAlgorithm を参照してください。
	@sa Session::SelectAlgorithm
	@endja
	*/
	namespace SelectSession
	{
		void * SelectTcpStreamSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamCamelliaSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamBlowfishSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamRijndaelSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamCamelliaLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamCamelliaZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamBlowfishLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamBlowfishZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamRijndaelLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectTcpStreamRijndaelZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);

		void * SelectUdpStreamSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamCamelliaSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamBlowfishSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamRijndaelSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamCamelliaLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamCamelliaZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamBlowfishLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamBlowfishZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamRijndaelLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectUdpStreamRijndaelZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);

		void * SelectLoopbackSessionSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionCamelliaSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionBlowfishSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionRijndaelSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionCamelliaLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionCamelliaZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionBlowfishLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionBlowfishZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionRijndaelLZPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
		void * SelectLoopbackSessionRijndaelZIPSession(vce::VCE * api, vce::Session * s, vce::Listener * l);
	}
}
